...
describe('PUT /requests/:requestId/disapprove', () => {
  ...
  it('should return 200 and request, when token is valid and request was disapproved', done => {
    chai
      .request(server)
      ...
      .end((req, res) => {
        ...
        res.body.should.have
          .property('message')
          .eql('Request disapproved successfully');
  ...
  it('should return 400, when requestId parameter is not valid', done => {
    chai
      .request(server)
      .put('/api/v1/requests/hs/disapprove')
      .set('Authorization', `Bearer ${superUserToken}`)
      .end((req, res) => {
        res.should.have.status(400);
        res.should.be.a('object');
