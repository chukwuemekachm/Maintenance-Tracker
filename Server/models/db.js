...
const migrateRequestDb = 'DROP TABLE IF EXISTS requests CASCADE; CREATE TABLE requests (id SERIAL NOT NULL PRIMARY KEY, title VARCHAR(50) NOT NULL, type VARCHAR(20) NOT NULL, description VARCHAR(255) NOT NULL, user_id int NOT NULL, status VARCHAR(20) NOT NULL DEFAULT \'pending\', createdat TIMESTAMP NOT NULL DEFAULT NOW(), updatedat TIMESTAMP NOT NULL DEFAULT NOW(), FOREIGN KEY (user_id) REFERENCES "users" (id) ON UPDATE CASCADE ON DELETE CASCADE );';
